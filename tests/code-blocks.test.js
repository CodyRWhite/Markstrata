...
test('line numbers are not part of the text content', () => {
  ...
  assert.match(html, /mdf-code--numbered/);
  assert.match(html, /<span class="mdf-code-ln" aria-hidden="true"><\/span>/);
});
...
test('the header can be turned off, leaving only the copy button', () => {
  ...
  assert.doesNotMatch(html, /mdf-code-header/);
  assert.match(html, /mdf-code-copy/);
});

test('trailing newline does not add an empty line', () => {
  const html = md.render('```\none\n```');
  assert.equal((html.match(/mdf-code-line"/g) || []).length, 1);
});
