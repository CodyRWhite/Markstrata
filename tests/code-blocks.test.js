...
test('line numbers are not part of the text content', () => {
  ...
  assert.match(html, /strata-code--numbered/);
  assert.match(html, /<span class="strata-code-ln" aria-hidden="true"><\/span>/);
});
...
test('the header can be turned off, leaving only the copy button', () => {
  ...
  assert.doesNotMatch(html, /strata-code-header/);
  assert.match(html, /strata-code-copy/);
});
