...
test('GitHub alert syntax becomes a callout', () => {
  ...
  assert.match(html, /<div class="mdf-callout" data-callout="note">/);
  assert.match(html, /<span class="mdf-callout-title-text">Note<\/span>/);
...
test('Obsidian syntax keeps a custom title', () => {
  ...
  assert.match(html, /<span class="mdf-callout-title-text">Try this instead<\/span>/);
});
...
test('foldable callouts render as details, - closed and + open', () => {
  ...
  assert.match(closed, /<details class="mdf-callout mdf-callout--foldable" data-callout="warning">/);
...
test('an unknown type stays an ordinary blockquote', () => {
  ...
  assert.doesNotMatch(html, /mdf-callout/);
});
...
test('a title-only callout leaves no empty paragraph', () => {
  ...
  assert.match(html, /<div class="mdf-callout-content"><\/div>/);
});
