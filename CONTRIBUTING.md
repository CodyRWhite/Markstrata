...
version in `package.json`. SPFx copies everything under `teams/` into the
`.sppkg`, so that zip ships inside the package.

**Sync to Teams** is then how it reaches the tenant, and the path and the name
above are what make that work: pressed on the Markstrata row in the app
catalog, SharePoint looks inside the package for `./teams/TeamsSPFxApp.zip`,
finds ours, and publishes that instead of generating a manifest of its own.
What it generates is an app whose short and full descriptions are both the word
"Markstrata", with no documentation link and a configuration page that cannot
pick a document, which is why the manifest is hand-written rather than why the
sync is avoided.
