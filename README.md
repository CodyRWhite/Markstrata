...
pages. Mermaid is a lazily loaded chunk, so pages without a diagram never download
it.
...
**Content** sets where the markdown comes from: typed into the web part, a file
...
**Appearance** covers theme, colour mode (light, dark, or follow the SharePoint page
...
**Features** covers the table of contents (left sidebar, right sidebar, above the
