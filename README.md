...
there. The Teams app manifest is written here rather than generated by the app
catalog - `npm run teams` builds `teams/TeamsSPFxApp.zip` from
`config/teams-app-manifest.json` and the icons in `teams/`, and SPFx carries
that zip inside the `.sppkg`. An administrator presses **Sync to Teams** in the
app catalog and SharePoint publishes it rather than generating a manifest of its
own. The `.sppkg` still has to be installed there, because that is where the tab
...
The site's pages, their order, which of its two sections each belongs to and the
navigation between them are declared once in `scripts/site.js`; adding one is an
entry there and a markdown file. Both the site and the package are built from
the same sources, so the site cannot drift from what the web part does. A page
that has to show something no single document can - the toolbar, the trail of
documents a reader has walked, the split editor - drops a panel of real markup
into itself, drawn at build time by the same renderer classes a SharePoint page
runs; `scripts/specimens.js` says how.
