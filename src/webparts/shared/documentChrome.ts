/**
 * .SYNOPSIS
 * The furniture around a document: the toolbar, the theme controls, the trail
 * of followed documents, the table of contents and the source footer.
 *
 * .DESCRIPTION
 * Two web parts draw a document, and they draw it differently: one renders
 * markdown, the other renders HTML someone else wrote. What surrounds the
 * document is the same in both, and a reader moving between them on one page
 * should not be able to tell which part they are looking at from the chrome.
 *
 * WHY THIS IS SEPARATE FROM THE RENDERERS
 * Because the chrome is built from the rendered article rather than from the
 * source. The contents list is collected by walking the headings that came out;
 * the reading time is counted from the prose that came out; the trail is built
 * from file names. None of it reads markdown, so none of it belongs to the
 * markdown renderer, and duplicating it into a second renderer would mean two
 * toolbars drifting apart one button at a time.
 *
 * WHAT IT DELIBERATELY DOES NOT DO
 * It does not render the document, and it does not own the host element's
 * layout beyond mounting the theme. The two renderers differ in exactly the
 * place they should differ - one puts sanitised HTML in an article, the other
 * may put it behind a shadow root or in a frame - and this class is not given
 * a say in that.
 *
 * It also holds no settings of its own. Everything it draws is decided by the
 * options handed in on every render, so a property pane change is a redraw and
 * never a stale field.
 *
 * WHICH WAY THIS FAILS
 * A control this draws that the host cannot service is the bad direction, so
 * each one is gated on the callback or the address that makes it work: no
 * share button without an address that would bring somebody back, no reload
 * without a file behind the page. A button that leads nowhere is worse than no
 * button.
 *
 * .USAGE
 *   import { DocumentChrome } from '../../shared/documentChrome';
 *   import { MARKDOWN_DOCUMENTS } from './documentLinks';
 *
 *   const chrome = new DocumentChrome(enhancer, callbacks, MARKDOWN_DOCUMENTS);
 *
 *   const host = chrome.mountHost(container, options);
 *   const toolbar = options.showToolbar ? chrome.buildToolbar(options) : undefined;
 *   // ... render the document into an article ...
 *   chrome.addToc(layout, article, host, options);
 *   chrome.fillReadingTime(toolbar, article);
 *   chrome.noteRenderedMode(options.resolvedMode);
 *
 * .NOTES
 * Since:     0.0.22.0
 * Ships in:  both web part bundles
 * Requires:  ContentEnhancer.ts, ThemeManager.ts, SharePointService.ts,
 *            backToTop.ts, icons.ts, documentNaming.ts
 */
...
// Type only: the chrome knows nothing about SharePoint at runtime, so it can be
// exercised in a plain browser page without the SPFx host.
import type { IFileMetadata } from '../markstrata/utils/SharePointService';
...
/**
 * What a document says about itself, whatever it said it in: markdown
 * frontmatter in one web part, a `<title>` in the other. The footer wants the
 * three values and does not care which syntax carried them.
 */
...
/**
 * Everything the chrome is drawn from. Both web parts' own option types extend
 * this, so a setting added here reaches both of them.
 */
...
export interface IChromeCallbacks {
  onReload: () => void;
  onShowVersions: () => void;
...
export class DocumentChrome {
  private enhancer: ContentEnhancer;
  private callbacks: IChromeCallbacks;
  /** The extensions this web part's own documents carry, kept off the crumbs. */
  private ownExtensions: RegExp;
  ...
  /** The colour mode this chrome last painted, or undefined before the first. */
  private lastMode: ResolvedMode | undefined;

  constructor(enhancer: ContentEnhancer, callbacks: IChromeCallbacks, ownExtensions: RegExp) {
  ...
  /**
   * The themed element everything else goes inside, marked with whether the
   * page around it is being edited.
   */
  public mountHost(container: HTMLElement, options: IChromeOptions): HTMLElement {
   ...
   */
  public buildOpenDocumentBar(options: IChromeOptions): HTMLElement {
    ...
    trail.forEach((name: string, index: number) => {
      ...
      if (index === trail.length - 1) {
        ...
        here.textContent = withoutExtension(name, this.ownExtensions);
        here.title = name;
        ...
        step.textContent = withoutExtension(name, this.ownExtensions);
  ...
  public buildToolbar(options: IChromeOptions): HTMLElement {
  ...
  /**
   * Fills in the reading time, which is measured from the rendered document
   * rather than from its source, so code and diagram source are not counted as
   * prose. That means it can only be done once the article exists.
   */
  public fillReadingTime(toolbar: HTMLElement | undefined, article: HTMLElement): void {
    ...
    /* Nothing to say about an empty document, and a lone clock says less than
       nothing. */
  ...
  public buildSourceInfo(metadata: IFileMetadata, description: IDocumentDescription): HTMLElement {
    const info: HTMLElement = document.createElement('div');
    info.className = 'strata-meta';

    /* A document that titled itself is better named by that than by its file
       name, which is often a slug. */
    const name: HTMLElement = document.createElement('span');
    name.textContent = description.title || metadata.name;
    ...
    const author: string = description.author || metadata.author;
    if (author) {
    ...
    (description.tags || []).forEach((tag: string) => {
  ...
  /**
   * Remembers the mode just painted, so the next render can tell a mode the
   * reader chose from one the page simply opened with.
   */
  public noteRenderedMode(mode: ResolvedMode): void {
    this.lastMode = mode;
  }
   ...
   * is neither.
   */
  public button(text: string, title: string, onClick: () => void,
  ...
  private buildThemeSwitcher(options: IChromeOptions): HTMLElement {
   ...
   */
  private buildModeToggle(options: IChromeOptions): HTMLButtonElement {
