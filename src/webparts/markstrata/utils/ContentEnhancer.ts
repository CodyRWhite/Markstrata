...
export class ContentEnhancer {
   ...
   * meant. Images have been resolved against the document's own folder since
   * the beginning; links never were. They are now, whatever they point at, so a
   * relative link to a PDF finds it too.
   *
   * Second, what happens when one is followed. A link to a .md file used to
   * hand the reader the file itself: SharePoint offers raw markdown, or
   * downloads it. `open` is called instead, and the web part loads that
   * document where it stands.
   *
   * Only a plain left click is taken. Ctrl, Shift, the middle button and the
   * rest are how people open things in a new tab, and taking those away would
   * be worse than what this fixes. The href is left on the element and pointing
   * at the real file, so those still work.
   */
  public followDocumentLinks(
    ...
    links.forEach((link: HTMLAnchorElement) => {
      ...
      /* Somebody else's site, and none of this applies. Read from the element
         rather than from the string: the browser has already worked out what
         the href means, which is the question being asked. */
      if (link.hostname && link.hostname !== window.location.hostname) {
        return;
      }
      ...
      if (!open || !ContentEnhancer.isDocument(target)) {
        return;
      }
  ...
  private static isDocument(href: string): boolean {
