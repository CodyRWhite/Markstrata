 ...
 *   - [x] done
 *
 * becomes a list marked `mdf-task-list` whose items carry `data-checked`, with
...
function taskListRule(state: IStateCore): void {
  ...
  for (let i: number = 0; i < tokens.length; i++) {
    ...
    token.attrJoin('class', 'mdf-task-item');
  ...
  taskLists.forEach((list: IToken) => list.attrJoin('class', 'mdf-task-list'));
}
...
export function taskListPlugin(md: IMarkdownIt): void {
  ...
  md.renderer.rules.mdf_task_checkbox = (tokens: IToken[], idx: number): string => {
    ...
    return (
      '<input class="mdf-task-checkbox" type="checkbox" disabled' +
      (checked ? ' checked' : '') +
      ' /> '
    );
  };
