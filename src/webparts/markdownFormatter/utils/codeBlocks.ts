...
export function renderCodeBlock(code: string, info: string, options: ICodeBlockOptions): string {
  ...
  const lines: string = splitHighlightedLines(highlighted.html)
    .map(
      (line: string) =>
        `<span class="mdf-code-line"><span class="mdf-code-ln" aria-hidden="true"></span>` +
        `<span class="mdf-code-line-text">${line}</span></span>`
    )
    .join('');

  const classes: string[] = ['mdf-code'];
  ...
  const copyButton: string =
    '<div class="mdf-code-actions">' +
    '<button type="button" class="mdf-code-btn mdf-code-copy" aria-label="Copy code to clipboard">' +
    COPY_ICON +
    '<span class="mdf-code-btn-label">Copy</span></button></div>';

  const header: string = options.showHeader
    ? '<div class="mdf-code-header">' +
      `<span class="mdf-code-lang">${escapeHtml(languageLabel(parsed.lang))}</span>` +
      (parsed.filename ? `<span class="mdf-code-filename">${escapeHtml(parsed.filename)}</span>` : '') +
      copyButton +
      '</div>'
    : copyButton;
  ...
  return (
    ...
    header +
    `<pre class="mdf-code-pre"><code class="${codeClass}">${lines}</code></pre>` +
    '</div>'
  );
}
