...
export class VersionPanel {
  ...
  public async open(host: HTMLElement, fileUrl: string): Promise<void> {
    ...
    panel.className = 'mdf-panel';

    const head: HTMLElement = document.createElement('div');
    head.className = 'mdf-panel-head';
  ...
  private buildRow(version: IVersionInfo, fileUrl: string): HTMLElement {
    ...
    row.className = 'mdf-version';

    row.appendChild(this.text('span', version.versionLabel, 'mdf-version-label'));

    const created: string = version.created ? new Date(version.created).toLocaleString() : '';
    row.appendChild(this.text('span', `${created} - ${version.createdBy}`, 'mdf-version-meta'));

    if (version.isCurrentVersion) {
      row.appendChild(this.text('span', 'Current', 'mdf-version-current'));
      return row;
    }
